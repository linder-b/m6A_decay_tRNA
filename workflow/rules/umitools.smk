...
#         r2=fastq_dir+'/{experiment}/{run}_1.fastq.gz'
#     output:
#         r1=fastq_dir+'/{experiment}/{run}_1.fastq.gz',
#         r2=fastq_dir+'/{experiment}/{run}_1.fastq.gz'
#     params:
...
rule umitools_dedup:
    ...
    output:
        bam=results_dir+'/{experiment}/star/{species}_{build}_{release}_{spikein}/{run}.Aligned.sortedByCoord.out.dedup.bam'
    params:
        config=lambda wildcards: get_params(wildcards.run, 'umitools_dedup')
    log:
        logs_dir+'/{experiment}/umitools/{species}_{build}_{release}_{spikein}/{run}.dedup.log'
    threads: 1
    conda:
        '../envs/sm_umitools.yaml'
    shell:
...
rule index_dedup:
    input:
        bam=results_dir+'/{experiment}/star/{species}_{build}_{release}_{spikein}/{run}.Aligned.sortedByCoord.out.dedup.bam'
    output:
        idx=results_dir+'/{experiment}/star/{species}_{build}_{release}_{spikein}/{run}.Aligned.sortedByCoord.out.dedup.bam.bai'
    conda:
