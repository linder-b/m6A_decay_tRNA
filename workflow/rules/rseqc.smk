...
rule rseqc_infer_experiment:
    input:
        ...
        bed=genome_dir+"/{species}_{build}_{release}/annotation.protein_coding.bed"
    output:
        log=logs_dir+'/{experiment}/rseqc/{species}_{build}_{release}/{run}.infer_experiment.txt'
    conda:
        '../envs/sm_rseqc.yaml'
    shell:
        """
...
rule rseqc_mismatch_profile:
    input:
        ...
        bed=genome_dir+"/{species}_{build}_{release}/annotation.protein_coding.bed"
    output:
        log=logs_dir+'/{experiment}/rseqc/{species}_{build}_{release}/{run}.mismatch_profile.r'
    params:
        prefix=logs_dir+'/{experiment}/rseqc/{species}_{build}_{release}/{run}'
    conda:
        '../envs/sm_rseqc.yaml'
    shell:
        """
