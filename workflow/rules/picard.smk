...
rule picard_dict:
    ...
    output:
        dict=genome_dir+"/{species}_{build}_{release}_{spikein}/genome.dict"
    threads:1
    params:
...
rule dropseq_create_intervals:
    input:
        ...
        reference_dict=genome_dir+"/{species}_{build}_{release}_{spikein}/genome.dict"
    params:
        memory=config['local']['memory'],
        reference_directory=genome_dir+"/{species}_{build}_{release}_{spikein}",
        tmp_directory=tmp_dir,
        prefix="annotation"
    output:
        intervals=genome_dir+"/{species}_{build}_{release}_{spikein}/annotation.rRNA.intervals"
    conda: '../envs/sm_dropseq_tools.yaml'
    shell:
...
rule picard_CollectRnaSeqMetrics:
    input:
        ...
        flat=genome_dir+"/{species}_{build}_{release}/annotation.refFlat",
        rRNA_intervals=genome_dir+"/{species}_{build}_{release}_{spikein}/annotation.rRNA.intervals"
    params:     
        tmp_directory=tmp_dir,
        memory=config['local']['memory']
    output:
        metrics=logs_dir+'/{experiment}/picard/{species}_{build}_{release}_{spikein}/{run}.rna_metrics.txt'
    conda: '../envs/sm_picard.yaml'
    shell:
        """
        picard \
...
rule picard_CollectInsertSizeMetrics:
    ...
    output:
        txt=logs_dir+'/{experiment}/picard/{species}_{build}_{release}_{spikein}/{run}.isize.txt',
        pdf=logs_dir+'/{experiment}/picard/{species}_{build}_{release}_{spikein}/{run}.isize.pdf'
    conda: '../envs/sm_picard.yaml'
    shell:
        #"""
        #picard \
