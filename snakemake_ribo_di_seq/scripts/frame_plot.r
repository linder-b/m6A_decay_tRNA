...
ggplot(data_subset)+geom_bar(aes(x=V1+0.4,y=V3,fill=factor(V4)),stat = "identity")+facet_wrap(~V2,scales="free",ncol=2)+coord_cartesian(xlim=c(-21,50))+scale_fill_manual(name="Class",values=color)+theme_bw()+ylab("Read counts")+xlab("Distance to start codon")+theme(legend.position="non")+scale_x_continuous(breaks=seq(-18,47,by=6))
