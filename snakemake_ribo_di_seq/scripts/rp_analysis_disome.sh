   ...
   awk  'BEGIN{FS="\t";OFS="\t"} $2==0{
    ...
    awk 'BEGIN{OFS="\t"}NR==FNR{
      ...
      if(NR%2==1){
        ...
        len[gene[NR-1]]=length($0)
      }
    }NR!=FNR && $1!~/^@/ && $3!="*"{
    
      sub(/M/,"",$6)
      if($0~/MD:Z:0[A-Z]/){
        if($6-1>="'$ARG_min_length'" && $6-1<="'$ARG_max_length'"){
          print $4+1-(len[$3]-21+1),$6-1
        }
      }else{
        if($6>="'$ARG_min_length'" && $6<="'$ARG_max_length'"){
          print $4-(len[$3]-21+1),$6
        }
      }
    }' $ARG_fasta $ARG_output/$name"_mapped.sam" | \
    ...
    }'  > $ARG_frame # read counts in frame
    ...
    if ! [ -x "$(command -v R)" ];then
       ...
       dir=${0/rp_analysis_disome.sh/}
       chmod +x $dir/frame_plot_disome.r
       R --vanilla --slave --args $ARG_frame $ARG_max_length $ARG_min_length < $dir/frame_plot_disome.r  # making the frame plot. see the pdf file. 
    fi
    End_mapping=`date +%s`
